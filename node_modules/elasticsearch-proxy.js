...
var ElasticSearchProxy = function(configuration, preRequest, postRequest) {
    ...
    var customConf = {};
    var proxyConf = {
        es_version: configuration.es_version,
        seeds : ["localhost:9200"],
        refresh : 1000,
        allow : {
            "GET" : ["(_search|_status|_mapping|_count)","/.+/.+/.+/_mlt","/.+/.+/.+]"],
    ...
    var proxyRequestHandler = function (req, res) {
        if (testFilters(req.method, req.url)) {
			...
			// check request headers for compressed data support
			comp_header_status = -1;
			if (req.headers['accept-encoding'] != undefined) {
				comp_header_status = req.headers['accept-encoding'].indexOf('gzip');
				//console.log("comp_header_status: " + comp_header_status);
			}
            ...
            req.on('end', function() {
                ...
                request.on('response', function(response) {
                    //console.log("came from elastic: "+ JSON.stringify(response.headers, null, 4));
                    //console.log("at status " + comp_header_status + " requested "+ req.httpVersion + " with headers " + JSON.stringify(req.headers, null, 4));
                    res.setHeader("Content-Type", "application/json; charset=UTF-8");
                    res.sendDate = true; // should add Date-header but doesn't

                    // Varnish and others should know we have option available
                    // not really sure if it matters though, seems to work fine without too
                    res.setHeader("Vary", "Accept-Encoding");

                    if (req.httpVersion === '1.1' && comp_header_status != -1) {
						res.setHeader("Content-Encoding", "gzip");
					} // no chunked option since content-length is sent anyway
					// nope, now chunked by default and Node.js adds chunked headers automatically
                    ...
                    if (isFunction(proxyConf.postRequest)) {
                        ...
                        response.on('end', function() {
							...
							if (res.getHeader('Content-Encoding') == 'gzip') {
								zlib.gzip(o, function(err, buffer) {
									//console.log("buffer: " + buffer.length);
                                    // turned out there is no need to send length, chunked works fine
                                    //res.setHeader("Content-Length", buffer.length);
									//console.log("Sending gzipped data..");
									res.write(buffer);
									res.end();
								});
							}
							else {
                                //res.setHeader("Content-Length", Buffer.byteLength(o, "utf8"));
                                // byteLength to have accurate byte length, just string length
								res.write(o);
								res.end();
							}
                        });
    ...
    var init = function() {
        ...
        httpClient = new HttpClient(proxyConf.seeds, proxyConf.es_version);
    };
 ...
 */
var HttpClient = function(seeds, es_version) {
    ...
    var _updateAllNodes = function(callback) {
        ...
        for (var i = 0; i < _numberOfSeeds; i++) {
            ...
            if (s && s.length == 2) {
                ...
                var request = c.request("GET", es_version == "0.90" ? "/_cluster/nodes": "/_nodes");
                request.on('response', function(response) {
